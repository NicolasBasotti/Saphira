...
CREATE TABLE "Classificação Idade" (
	codclafil SERIAL NOT NULL, 
	nomclafil varchar(40) NOT NULL, 
	PRIMARY KEY (codclafil));

COMMENT ON COLUMN "Classificação Idade".codclafil IS 'Código da classificação do filme';
COMMENT ON COLUMN "Classificação Idade".nomclafil IS 'Nome da classificação do filme';
...
CREATE TABLE "Genero do filme" (
	codgenfil SERIAL NOT NULL, 
	nomgenfil varchar(40) NOT NULL, 
	PRIMARY KEY (codgenfil));

COMMENT ON COLUMN "Genero do filme".codgenfil IS 'Código do gênero do filme';
...
ALTER TABLE Filme ADD CONSTRAINT FKFilme434784 FOREIGN KEY (codgenfil) REFERENCES "Genero do filme" (codgenfil);
ALTER TABLE Filme ADD CONSTRAINT FKFilme554609 FOREIGN KEY (codclafil) REFERENCES "Classificação Idade" (codclafil);
